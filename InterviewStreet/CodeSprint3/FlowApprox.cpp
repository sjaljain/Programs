//REFER to ASSIGN4 spoj
//This code works for a general graph
//Faster code may be required for biparite graph
...
int main( int argc, char* argv[] ) {
	...
	while(tc--){
		...
		rep(i,m){
			rep(j,n){
				int c=sc.nextInt();
				addEdge(i, m+j, 60000, c);//addEdge(source,destination,capacity,cost)
			}
		}

		pii res=maxFlowOfMinCost(s,t);//compute min cost
