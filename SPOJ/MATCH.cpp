...
int main(int argc, char** argv) {
 ios_base::sync_with_stdio(false);
    ...
    while(T--)
    {
       ...
       BipartiteMatching* bpm = new BipartiteMatching(310,310);
       //406 vertices in set A, 206 in set B // keep some extra
       bpm->Reset();
       //Add the vertices index using the following command
       for(int i=0;i<N;i++)
       for(int j=0;j<N;j++)
       if(v[i][j]=='1')  bpm->AddEdge(i,j);
       // calculate maximum bipartite matching
       if(bpm->maxMatching()%2==0)cout<<"Even"<<endl;
       else cout<<"Odd"<<endl;
    }
