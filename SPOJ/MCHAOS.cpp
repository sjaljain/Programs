...
bool cmp1( const rijec &A, const rijec &B ) { if(strcmp(A.s1,B.s1)<0) return true; else return false; }
bool cmp2( const rijec &A, const rijec &B ) { if(strcmp(A.s2,B.s2)<0) return true; else return false; }
...
int main( void ) {
   ...
   for( int i = 0; i < n; ++i ) {
      ...
      scanf( "%s", A.s1 );
      int L=strlen(A.s1);
      for(int i=0;i<L;i++)A.s2[i]=A.s1[L-i-1];
      A.s2[L]='\0';
      dict.push_back( A );
   }
